...
# model create
def model_create():
    model = smp.Unet(encoder_name=params['model'], encoder_weights=None, in_channels=1, classes=2)
    model.to(device)
    return model
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (images, target) in enumerate(stream, start=1):
        ...
        torch.save(target.int(), 'target.pt') 
...
# checkpoint setting
project_name = f"2DDenseNet121Unet - lr_{params['lr']} - FTL (only test code)"
project_folder = f'2021.11.23.t1 - 2DDenseNet121Unet (only test code)'
