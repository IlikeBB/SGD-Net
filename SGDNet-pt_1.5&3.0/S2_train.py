...
if True:
    ...
    nii_3t_train = sorted([i for i in os.listdir(os.path.join('./dataset/S2_data1.5&3.0/','train'))])
    nii_3t_valid = sorted([i for i in os.listdir(os.path.join('./dataset/S2_data1.5&3.0/','valid'))])

# Subject Function Building
def tio_process(nii_3t_, table_3t_, basepath_='./dataset/S2_data1.5&3.0/train/'):
    subjects_ = []
...
if True: #model record
    params = {
        ...
        "adjust01": "CosineAnnealingLR(mt=epoch/2), loss funciton -> focal loss:[0.8,2]",
        # "adjust01": "None Learning rate decay, loss funciton -> focal loss:[0.8,2]",
        "adjust02": "learning rate 0.003 -> 0.001, Used SGD optimizer",
        # "augment": "RandomFlip['AP'], RandomElasticDeformation",
        "augment": "RandomFlip, RandomElasticDeformation, RandomAffine",
        "fixing": "remove torchio normalized -> data augmentation in build dataset, resize all MRI to 384 scale"
        }

if True: #data augmentation, dataloader, 
    training_subjects = tio_process(nii_3t_train, table_3t_train, basepath_ = './dataset/S2_data1.5&3.0/train/')
    validation_subjects = tio_process(nii_3t_valid, table_3t_valid, basepath_ = './dataset/S2_data1.5&3.0/valid/')
    ...
    # checkpoint setting
    project_name = f"{params['type']} - {params['model']}{params['model_depth']} - lr_{params['lr']} - FL"
    project_folder = f"2021.11.25.t2 - 3DResNet18 - {params['type']}"
