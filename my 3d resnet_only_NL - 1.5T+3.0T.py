...
from utils.ImageDataGenerator import LN_generateor
...
def data_loder(valid_data='3.0T'):
    ...
    if valid_data=='3.0T':
        ...
        # 1.5 T nii data processing
        valid_y_15T = np.load(MRI_nii_folder_path + 'T1_5_onehot_valid.npy')
        X_train = np.load(MRI_nii_folder_path+f'/image_mask_arr_3.0T_384_train.npy')
        X_valid = np.load(MRI_nii_folder_path+f'/image_mask_arr_1.5T_384_valid.npy')
        y_train = np.array(train_y_30T).astype(np.int8)
        y_valid = np.array(valid_y_15T).astype(np.int8)
        path_valid = valid_path_15T

    elif valid_data=='Mix':
        # 3.0 T + 1.5 T nii data processing
        valid_y_15T = np.load(MRI_nii_folder_path + 'T1_5_onehot_valid.npy')
        valid_y_30T = np.load(MRI_nii_folder_path + 'T3_0_onehot_valid.npy')
        X_train = np.load(MRI_nii_folder_path+f'/image_mask_arr_3.0T_384_train.npy')
        X_valid1 = np.load(MRI_nii_folder_path+f'/image_mask_arr_3.0T_384_valid.npy')
        X_valid2 = np.load(MRI_nii_folder_path+f'/image_mask_arr_1.5T_384_valid.npy')
        y_train = np.array(train_y_30T).astype(np.int8)
        y_valid1 = np.array(valid_y_30T).astype(np.int8)
        y_valid2 = np.array(valid_y_15T).astype(np.int8)
        path_valid = valid_path_15T
        X_valid = np.concatenate((X_valid1,X_valid2))
...
def trainer(valid_data='3.0T'): 
    ...
    #     model build
    #     model = model_loader(weight_zero = cw[0], weight_one = cw[1])
    model = model_loader()

    #     data loader
    path_valid, X_train, X_valid, y_train, y_valid= data_loder(valid_data='Mix')
    ...
    train_generator, valid_generator = LN_generateor(X_train, y_train, X_valid, y_valid, batch_size)
...
if __name__ == '__main__':
    def load_config(config_name):
        ...
        return config
    config = load_config("utils/model_config_NL.yaml")
    # Model SETTINGS
    ...
    checkpoint_name = f"3DResnet18-{nii_size}-{class_n}-epochs_{epochs}-lr_{lr}-batch_{batch_size}-FL{config['focal_alpha']}-{date_name}"
    ...
    valid_path_15T = np.load(MRI_nii_folder_path + 'T1_image_mask_path_valid.npy')
