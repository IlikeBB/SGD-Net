...
import segmentation_models as sm, albumentations as A, tensorflow_addons as tfa
...
def dataloader():
    ...
    X_valid3 = np.expand_dims(np.load(MRI_nii_folder_path + config["valid_img_p3"]), axis=-1)
    y_valid3 = np.expand_dims(np.load(MRI_nii_folder_path + config["valid_msk_p3"]), axis=-1)
    X_valid1 = np.expand_dims(np.load(MRI_nii_folder_path + config["valid_img_p1"]), axis=-1)
    y_valid1 = np.expand_dims(np.load(MRI_nii_folder_path + config["valid_msk_p1"]), axis=-1)
    X_valid = np.concatenate((X_valid3, X_valid1))
    y_valid = np.concatenate((y_valid3, y_valid1))
    del X_valid3, y_valid3, X_valid1, y_valid1
...
def model_loder():
    ...
    snapshot = get_callbacks(nb_epochs=epochs,nb_snapshots=1,init_lr=lr)
    model = Unet(config['BACKBONE'], encoder_weights=None, input_shape=(None, None, 1))
    optimizer=tfa.optimizers.Lookahead(tfa.optimizers.RectifiedAdam(lr=lr))
    # optimizer = 'adam'
...
def trainer():
    ...
    # snapshot = SnapshotCallbackBuilder(nb_epochs=epochs, nb_snapshots=1, init_lr=lr)
    snapshot = get_callbacks(nb_epochs=epochs, nb_snapshots=1, init_lr=lr)
    # model.load_weights("checkpoints/S1-segment-2021.05.03(NL_data)/logs_0503.2_CK/saved-model-epoch_45-acc_0.999-dice_0.665-iou_0.580.hdf5")
    print('-'*30,'\nFitting model...\n','-'*30)
...
if __name__ == '__main__':
    ...
    checkpoint_name = f"2DDenseUnet-{nii_size}-epochs_{epochs}-lr_{lr}-batch_{batch_size}-{date_name}"
